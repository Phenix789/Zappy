...
** Last update Sat Jun  2 00:30:03 2012 quentin duval
*/
...
void session_execute(t_client *client, char *cmd)
{
  ...
  if (!client->player)
    {
      logger_message("[SESSION] Search player for team '%s'", cmd);
      game_set_player(client, cmd);
    }
  else
    {
      ...
      if (exec)
	{
	  ...
	  if (client->busy)
	    session_command_stack(client, cmd);
	  else
	    {
